...
<div id="edit" style="padding:20px"> 
    ...
    <div style="padding-left:20px" v-loading="loading">
      <el-form ref="form" :model="purchaseData" label-position="top" :rules="rules">
        ...
        <span style="color:#606266;position:absolute;left:5px;top:5px;font-size:20px" >{{i+1}}.</span>  
        <i  v-if="!formData" class="el-icon-close" style="position:absolute;right:5px;top:5px;font-size:16px" @click="handleClose(i)"></i>
        <el-col :span="12">
...
export default {
  ...
  mounted() {
    ...
    if(this.$route.params.id=="edit"){
       ...
       if(this.formData.paymentTime){
         ...
         this.purchaseData.paymentTime =  new Date(this.formData.paymentTime).getTime();
       }
   ...
   let promiseTwo =  this.axios({
    ...
    let promiseThree = this.axios({
      url: "api.myfbmanage.com:8000/data-server/purchase/currencies",
      method: "post",
      data: {
        token: this.token,
        type:'purchase'
  ...
  methods: {
      ...
      let data = [];
        ...
        this.loadsh.each(this.purchaseData.firstPartData,(v,i)=>{
          ...
          obj = {
            ...
            currency:v.currency,
            purchasedTime:this.moment(this.purchaseData.purchasedTime).format('YYYY-MM-DD HH:mm:ss'),
          ...
          if(this.treasureSwitch){
            obj.paymentTime = this.moment(this.purchaseData.paymentTime).format('YYYY-MM-DD HH:mm:ss');
          }  
    ...
    handleClose(index) {
      if(this.purchaseData.firstPartData.length==1)
      return;
