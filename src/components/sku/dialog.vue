...
export default {
        ...
        validator(rule, value, callback){
          ...
          if(!rules.test(value)){
            callback(new Error('SKU結構有誤'))
              ...
              that.axios({
              url: "http://60.251.57.138:8000/data-server/sku/similarimg",
              method: "post",
              data:{
                sku:value,
                token:rule.token
  ...
  mounted() {
         axios({
              url:"http://60.251.57.138:8000/data-server/sku/cat",
               method: "post",
              data:{
                token:this.token
              }
    ...
    if (this.title == "編輯") {
      ...
      this.url = "http://60.251.57.138:8000/data-server/sku/update";       
    } else {
      this.url = "http://60.251.57.138:8000/data-server/sku/add";
    }
  },
  watch: {
  ...
  methods: {
    ...
    handleAuto() {
      axios({
        url: "http://60.251.57.138:8000/data-server/sku/newindex",
        method: "post",
        data: {
      ...
      this.$refs["form1"].validate((action, dom) => {
        ...
        if(this.title == "編輯"){
          ...
          if (action) {            
            ...
            obj.image = this.form.image;
            this.isLoading = true;
            var formData = new FormData();
            for(let k in obj){
              if(k=='image'){
                formData.append(k, this.image);  
              }
