      ...
      >
        ...
        <tbody>
          ...
          >
            ...
            <td>{{v.paymentFee | formToEmpty}}</td>
            <td>{{v.fineFee | formToEmpty}}</td>
            <td>{{add(v.paymentFee,add(v.fineFee, add(v.otherFee, add(v.customerServiceFee, add(v.fees,v.advertising))))).toFixed(2) | formToEmpty}}</td>
          </tr>
        </tbody>
      </table>
    </div>
        ...
        axios({
            ...
            _.each(data, (v, key) => {
                ...
                let dataObj = v.reduce((accumulator, currentValue) => {
                    ...
                    // 收款手續費
                    if (currentValue.financialType == "收款手續費") {
                        accumulator.paymentFee = this.add(
                            accumulator.paymentFee,
                            currentValue.amount
                        );
                    }
                    // 罚款
